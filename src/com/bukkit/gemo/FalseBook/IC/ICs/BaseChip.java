...
public class BaseChip
{
  ...
  public boolean hasInput(Block eventBlock, ArrayList<Location> positions) {
    if ((hasInput1()) && 
      (BlockUtils.LocationEquals(eventBlock.getLocation(), positions.get(0)))) {
      return true;
    }
    if ((hasInput2()) && 
      (BlockUtils.LocationEquals(eventBlock.getLocation(), positions.get(1)))) {
      return true;
    }

    return (hasInput3()) && 
      (BlockUtils.LocationEquals(eventBlock.getLocation(), positions.get(2)));
  }
